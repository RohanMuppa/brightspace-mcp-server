...
/**
 * Turndown has no rule for <script>, <style> or <noscript>, so it falls back to
 * emitting their text: a D2L description carrying an inline stylesheet or an
 * analytics snippet put raw CSS and JavaScript source into the markdown the
 * model reads. None of it is content.
 */
...
/**
 * Turndown has no table rules either, and its fallback treats every cell as a
 * standalone block. A syllabus grading table came out as a flat run of
 * paragraphs -- "Homework", "30%", "Exams", "70%" -- with nothing left to say
 * which weight belonged to which item. Emit GFM pipe tables so the pairing
 * survives into the markdown a model is asked to read.
 */
...
turndownService.addRule("tableRow", {
  ...
  replacement: (content, node) => {
    ...
    // without one the whole block renders as a paragraph and the columns are
    // lost again.
